...
try:
    f = open("data/shard.data", "r")
    f.close()

except:
    print("Creating shard file.")
    f = open("data/shard.data", 'w+')
    f.write("0")
    f.close()

shard_count = int(open("data/shard_count.data", "r").read())
shard = int(open("data/shard.data", "r").read())
print("Booting shard {}".format(str(shard)))
f = open("data/shard.data", "w")
...
class FilterBot(discord.Client):
    def __innit__(self):
        ...
        self.owner = ""
        self.prefix = ">"
        super().__innit__(shard_id=shard, shard_count=shard_count)
        self._setup_logging()
